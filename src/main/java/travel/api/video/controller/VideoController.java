...
public class VideoController {
    ...
    @PostMapping("/list")
    public WorkResponse getVideoList() {
        log.info("接收到访问视频页面请求");
        WorkStatus workStatus = null;
        WorkResponse workResponse;
        JSONObject json = null;
        try {
            json = videoService.getVideoList();
            log.info("访问视频页面请求成功");
    ...
    @PostMapping("/get")
    public WorkResponse getVideoDetail(@RequestBody VideoInfo videoInfo) {
        log.info("接收到访问视频详情请求");
        WorkStatus workStatus = null;
        WorkResponse workResponse;
        JSONObject json = null;
        try {
            json = videoService.getVideoDetail(videoInfo);
            log.info("访问视频详情请求成功");
        } catch (WorkException work) {
            workStatus = work.getWorkStatus();
            log.info("访问视频详情请求失败，失败原因：" + work.getExceptionMsg());
        } catch (Exception e) {
            workStatus = WorkStatus.FAIL;
            e.printStackTrace();
            log.info("访问视频详情请求失败，失败原因：系统出现异常");
        } finally {
            workResponse = new WorkResponse(workStatus, json);
        }
        return workResponse;
